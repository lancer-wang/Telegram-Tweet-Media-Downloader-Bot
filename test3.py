...
#         cur.close()
#         con.close()
    con2 = sqlite3.connect("/cobudy/pics.db")
    return con2
...
db = get_db3()
cursor = db.cursor()
last_sql = "SELECT SUM(CASE WHEN status = 1 THEN 1 ELSE 0 END) AS status_one_count, SUM(CASE WHEN status = 0 THEN 1 ELSE 0 END) AS status_zero_count FROM omega_pixiv_illusts_new;"
